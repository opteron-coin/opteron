...
BOOST_FIXTURE_TEST_SUITE(asset_reissue_tests, BasicTestingSetup)
    ...
    CNewAsset asset1("opteronASSET", CAmount(100 * COIN), 8, 1, 0, "");
    ...
    BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("opteronASSET"), "Map Reissued Asset should contain the asset \"opteronASSET\"");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("opteronASSET", Params().GlobalBurnAddress())) == CAmount(101 * COIN), "Reissued amount wasn't added to the previous total");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddresses.at("opteronASSET").count(Params().GlobalBurnAddress()), "Reissued address wasn't in the map");

    // Get the new asset data from the cache
    CNewAsset asset2;
    BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("opteronASSET", asset2), "Failed to get the asset2");
    ...
    BOOST_CHECK_MESSAGE(asset2.strName == "opteronASSET", "Asset2: Asset name is wrong");
    ...
    undoBlockData.emplace_back(std::make_pair("opteronASSET", CBlockAssetUndo {true, false, "", 0}));
    ...
    CNewAsset asset3;
    BOOST_CHECK_MESSAGE(cache.GetAssetMetaDataIfExists("opteronASSET", asset3), "Failed to get the asset3");
    ...
    BOOST_CHECK_MESSAGE(asset3.strName == "opteronASSET", "Asset3: Asset name is wrong");
    ...
    BOOST_CHECK_MESSAGE(cache.mapReissuedAssetData.count("opteronASSET"), "Map of reissued data was removed, even though changes were made and not databased yet");
    BOOST_CHECK_MESSAGE(cache.mapAssetsAddressAmount.at(make_pair("opteronASSET", Params().GlobalBurnAddress())) == CAmount(100 * COIN), "Assets total wasn't undone when reissuance was");
    }
    ...
    BOOST_AUTO_TEST_CASE(reissue_isvalid_test) {
        ...
        CReissueAsset reissue3("opteronASSET", CAmount(1 * COIN), 7, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue4("opteronASSET", CAmount(1 * COIN), -1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));

        BOOST_CHECK_MESSAGE(reissue4.IsValid(error, cache), "Reissue4 wasn't valid");

        // Create a new asset object with units of 0
        CNewAsset asset2("opteronASSET2", CAmount(100 * COIN), 0, 1, 0, "");
        ...
        CReissueAsset reissue5("opteronASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
        ...
        CReissueAsset reissue6("opteronASSET2", CAmount(1 * COIN), 1, 1, DecodeIPFS("QmacSRmrkVmvJfbCpmU6pK72furJ8E8fbKHindrLxmYMQo"));
