...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** opteron START */
    case OP_opteron_ASSET              : return "OP_opteron_ASSET";
    /** opteron END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_opteron_ASSET) { // OP_opteron_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == opteron_R) { // Check to see if opteron starts at 27 ( this->size() < 105)
                if ((*this)[28] == opteron_V)
                    if ((*this)[29] == opteron_N)
                        index = 30;
            } else {
                if ((*this)[28] == opteron_R) // Check to see if opteron starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == opteron_V)
                        if ((*this)[30] == opteron_N)
                            index = 31;
            }

            if (index > 0) {
                nStartingIndex = index + 1; // Set the index where the asset data begins. Use to serialize the asset data into asset objects
                if ((*this)[index] == opteron_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == opteron_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == opteron_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == opteron_R) {
