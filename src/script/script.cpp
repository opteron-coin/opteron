...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** ASTRAL START */
    case OP_ASTRAL_ASSET              : return "OP_ASTRAL_ASSET";
    /** ASTRAL END */
...
{
    if (this->size() > 30) {
        if ((*this)[25] == OP_ASTRAL_ASSET) { // OP_ASTRAL_ASSET is always in the 25 index of the script if it exists
            int index = -1;
            if ((*this)[27] == ASTRAL_R) { // Check to see if ASTRAL starts at 27 ( this->size() < 105)
                if ((*this)[28] == ASTRAL_V)
                    if ((*this)[29] == ASTRAL_N)
                        index = 30;
            } else {
                if ((*this)[28] == ASTRAL_R) // Check to see if ASTRAL starts at 28 ( this->size() >= 105)
                    if ((*this)[29] == ASTRAL_V)
                        if ((*this)[30] == ASTRAL_N)
                            index = 31;
            }
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == ASTRAL_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == ASTRAL_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == ASTRAL_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == ASTRAL_R) {
