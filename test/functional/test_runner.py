...
`test/functional/test_framework/test_framework.py:opteronTestFramework.main`.
...
BASE_SCRIPTS= [
    ...
    # 'p2p_fingerprint.py',         TODO - fix mininode rehash methods to use x20r
    # 'p2p_invalid_block.py',       TODO - fix mininode rehash methods to use x20r
    # 'p2p_invalid_tx.py',          TODO - fix mininode rehash methods to use x20r
    # 'feature_segwit.py',          TODO - fix mininode rehash methods to use x20r
    # 'p2p_sendheaders.py',         TODO - fix mininode rehash methods to use x20r
    # 'feature_nulldummy.py',       TODO - fix mininode rehash methods to use x20r
    # 'mining_basic.py',            TODO - fix mininode rehash methods to use x20r
    # 'feature_dersig.py',          TODO - fix mininode rehash methods to use x20r
    # 'feature_cltv.py',            TODO - fix mininode rehash methods to use x20r
...
def main():
    ...
    tmpdir = "%s/opteron_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_opterond = config["components"].getboolean("ENABLE_RAVEND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/opteronProject/opteroncoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/opteronProject/opteroncoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_opterond):
        print("No functional tests to run. Wallet, utils, and opterond must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if opterond is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "opterond"]) is not None:
            print("%sWARNING!%s There is already a opterond process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "RAVEND" not in os.environ:
        os.environ["RAVEND"] = build_dir + '/src/opterond' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/opteron-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `opteron-cli help` (`rpc_interface.txt`).
