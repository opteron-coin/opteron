...
`test/functional/test_framework/test_framework.py:AstralTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/astral_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    enable_astrald = config["components"].getboolean("ENABLE_RAVEND")

    if config["environment"]["EXEEXT"] == ".exe" and not args.force:
        # https://github.com/AstralProject/Astralcoin/commit/d52802551752140cf41f0d9a225a43e84404d3e9
        # https://github.com/AstralProject/Astralcoin/pull/5677#issuecomment-136646964
        print("Tests currently disabled on Windows by default. Use --force option to enable")
        sys.exit(0)

    if not (enable_wallet and enable_cli and enable_astrald):
        print("No functional tests to run. Wallet, utils, and astrald must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    # Warn if astrald is already running (unix only)
    try:
        if subprocess.check_output(["pidof", "astrald"]) is not None:
            print("%sWARNING!%s There is already a astrald process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "RAVEND" not in os.environ:
        os.environ["RAVEND"] = build_dir + '/src/astrald' + exeext
        os.environ["RAVENCLI"] = build_dir + '/src/astral-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `astral-cli help` (`rpc_interface.txt`).
